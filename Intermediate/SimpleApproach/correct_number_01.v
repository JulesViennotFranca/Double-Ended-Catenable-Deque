...
Fixpoint packet_value {color} (packet : packet color) : nat :=
  match packet with 
  | Hole => 0
...
(* The function [thickness] determines the power of 2 associated with the bit 
   following the last '1' in a headed list of ones. For a headed list of 
   ones, denoted as [ho], thickness ho is equal to 2 ** (1 + length ho). 
...
Fixpoint thickness {color} (packet : packet color) : nat :=
  match packet with
  | Hole => 1
...
#[global, export] Obligation Tactic :=
   try solve [ eauto | simpl; lia ].
