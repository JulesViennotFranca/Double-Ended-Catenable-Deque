...
Inductive packet (A : Type) (hlvl : nat) : nat -> color -> Type :=
  ...
  | PRed {tlvl : nat} {GP YP RP Y GS YS RS} :
    buffer A hlvl (Mix GP YP RP) ->
    packet A (S hlvl) tlvl (Mix NoGreen Y NoRed) ->
    buffer A hlvl (Mix GS YS RS) ->
    packet A hlvl tlvl red.
Arguments Hole {A hlvl}.
Arguments PGreen {A hlvl tlvl Y}.
Arguments PYellow {A hlvl tlvl GP YP Y GS YS}.
Arguments PRed {A hlvl tlvl GP YP RP Y GS YS RS}.
...
packet_left_seq (PGreen buf1 p _) := buffer_seq buf1 ++ packet_left_seq p;
packet_left_seq (PYellow buf1 p _) := buffer_seq buf1 ++ packet_left_seq p;
packet_left_seq (PRed buf1 p _) := buffer_seq buf1 ++ packet_left_seq p.
...
packet_right_seq (PGreen _ p buf2) := packet_right_seq p ++ buffer_seq buf2;
packet_right_seq (PYellow _ p buf2) := packet_right_seq p ++ buffer_seq buf2;
packet_right_seq (PRed _ p buf2) := packet_right_seq p ++ buffer_seq buf2.
...
chain_seq (CGreen p cd) :=
  packet_left_seq p ++ chain_seq cd ++ packet_right_seq p;
...
Equations make_red {A tlvl} {G1 Y1 R1 Y2 G3 Y3 R3}
  (buf1 : buffer A 0 (Mix G1 Y1 R1))
  (p : packet A 1 tlvl (Mix NoGreen Y2 NoRed))
  (buf2 : buffer A 0 (Mix G3 Y3 R3))
