...
type ('a, 'b, 'color) packet =
  ...
  | Green_node : ('a, green) buffer
                * ('a * 'a, 'b, nogreen * _ * nored) packet
                * ('a, green) buffer
              -> ('a, 'b, green) packet

  (* Packet starting with one. *)
  | Yellow_node : ('a, _ * _ * nored) buffer
                * ('a * 'a, 'b, nogreen * _ * nored) packet
                * ('a, _ * _ * nored) buffer
                -> ('a, 'b, yellow) packet
...
= fun prefix1 buf suffix1 ->
  ...
  | Ok p1, Underflow opt ->
      ...
      | Some (Any rest, cd), _ ->
          Chain (G, Green_node (p1, Hole, suffix23 cd opt), Ending rest)
      end
  ...
  | Underflow opt, Ok s1 ->
      ...
      | Some (cd, Any rest), _ ->
          Chain (G, Green_node (prefix23 opt cd, Hole, s1), Ending rest)
      end
  ...
  | Underflow p1, Underflow s1 ->
      ...
      (* If the following buffer has enough elements to be sandwich, we can
          ...
          suffix. *)
      | Sandwich (ab, rest, cd) ->
          Chain (G, Green_node (prefix23 p1 ab, Hole, suffix23 cd s1), Ending rest)
  ...
  | Overflow (p1, ab), Ok s1 ->
      ...
      Chain (G, Green_node (p1, Hole, s1), buf)
  ...
  | Underflow opt, Overflow (s1, ab) ->
      ...
      Chain (G, Green_node (prefix23 opt cd, Hole, s1), Ending center)

  (* This is symmetrical to the previous case. *)
  | Overflow (p1, ab), Underflow opt ->
      let center, cd = prefix_rot ab buf in
      Chain (G, Green_node (p1, Hole, suffix23 cd opt), Ending center)
  ...
  | Overflow (p1, ab), Overflow (s1, cd) ->
      ...
      Chain (G, Green_node (p1, Yellow_node (p2, Hole, B1 cd), s1), Ending rest)
...
= function
  ...
  | Chain (R, Red_node (p1, Hole, s1), Ending buf) ->
...
let yellow p1 child s1 chain =
  T (Chain (Y, Yellow_node (p1, child, s1), ensure_green chain))
