...
(* For a type [N], an instance of [NatRepresentation N] means that [N] can 
   represent natural numbers. This type class only contains one function, 
   [value], that returns the integer represented by an element of type [N]. *)

Class NatRepresentation (N : Type) := {
  value : N -> nat
}.
...
(* Packets can represent integers, therefor we can add an instance of 
   NatRepresentation for packets. *)

Instance NatPacket {color} : NatRepresentation (packet color) := {|
  value := packet_value
|}.
...
(* Colored numbers can represent integers, an instance of NatRepresentation is
   created for them. *)

Instance NatColoredNbr {color} : NatRepresentation (colored_nbr color) := {|
...
(* The goal of [nbr] is to represent integers, we obviously add an instance of 
   NatRepresentation for nbr. *)

Instance NatNbr : NatRepresentation nbr := {|
  value := nbr_value
|}.
...
Proof.
  ...
  (* Not the most beautiful proof, but I had troubles unifying the value
     function of NatColoredNbr and colored_nbr_value, which are the same. *)
