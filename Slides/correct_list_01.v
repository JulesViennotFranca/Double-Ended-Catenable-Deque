...
Lemma flatten_app {A : Type} (l1 l2 : list (A * A)) : flatten (l1 ++ l2) = flatten l1 ++ flatten l2.
Proof.
...
Fixpoint packet_value {A : Type} {n : nat} {color : color} (p : packet A n color) : list A := 
  match p with 
  | Hole => []
...
Fixpoint colored_list_value {A : Type} {color} (clist : colored_list A color) : list A :=
  match clist with 
  | Null => []
