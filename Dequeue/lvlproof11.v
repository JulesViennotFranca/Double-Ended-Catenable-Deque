...
(* Transforms a list of higher level into a list of one level lower, preserving 
   the order. *)
...
(* A generalization of [flattenp], it is simply iterated m times.  *)

Equations flattenp_plus {A n} (m : nat) : list (prodN A (m + n)) -> list (prodN A n) := 
flattenp_plus 0 := id;
flattenp_plus (S m) := (flattenp_plus m) ∘ flattenp.
Arguments flattenp_plus {A n m}.
...
(* Get the simple element of type A stored in a prodN A 0. *)
...
(* In the following, when talking about structures, we will wrote n-struct for
...
(* A new tactic [flattenp_eq H] is introduced. If H contains an equality on list
   at a certain high level, it applies flattenp on the equality to lower its 
   level. It then try to rewrite our new lowered equality in the goal.  *)
...
(* A simple equality on natural numbers, it is already in the standard librairy
   but I needed it to be transparent. *)

Equations plus_n_Sm : forall n m, n + S m = S (n + m) :=
plus_n_Sm 0 m := eq_refl;
plus_n_Sm (S n) m := f_equal S (plus_n_Sm n m).

(* Transforms a list of lower level into a list of one level higher, preserving 
   the order. If the list as a odd number of elements, the last one is dropped. *)
...
(* For all list of at least level 1, applying flattenp then inv_flattenp returns
   the same list. *)
...
(* Proof that we can apply flattenp in an eq_rect equality. *)

Lemma flattenp_eq_rect {A n m} (e : n = m) (l : list (prodN A (S n))) (l' : list (prodN A (S m))) : 
...
Proof.
  ...
  assert (forall n l, f n l = flattenp (inv_flattenp l)) as Hf. { intros; unfold f; reflexivity. }
  rewrite <-Hf.
  rewrite <-(map_subst f).
  unfold f, compose.
  reflexivity. 
...
(* Proof of the commutativity of flattenp_plus and flattenp when we have an 
   eq_rect equality on a (S n + m)-list and a (n + S m) list. *)

Lemma flattenp_Sn_m {A n m} (l : list (prodN A (S n + m))) (l' : list (prodN A (n + S m))) : 
    l = eq_rect _ (fun n => list (prodN A n)) l' _ (plus_n_Sm n m) ->
    flattenp_plus (flattenp l) = flattenp (flattenp_plus l').
Proof.
  intro e.
  induction n; simp flattenp_plus.
  - rewrite <-eq_rect_eq in e. destruct e. 
    unfold id. reflexivity.
  - apply IHn.
    apply flattenp_eq_rect.
    assumption. 
Qed.

(* Proof of the commutativity of flattenp_plus and flattenp when we have an 
   eq_rect equality on a (n + S m)-]ist and a (S n + m)-list. *)

Lemma flattenp_n_Sm {A n m} (l : list (prodN A (n + S m))) (l' : list (prodN A (S n + m))) :
    l = eq_rect _ (fun n => list (prodN A n)) l' _ (eq_sym (plus_n_Sm n m)) ->
    flattenp (flattenp_plus l) = flattenp_plus (flattenp l').
Proof.
  intro e.
  induction n; simp flattenp_plus.
  - rewrite <-eq_rect_eq in e. destruct e.
...
#[local] Obligation Tactic := 
  ...
  unfold compose;
  (* Use previous lemmas to simplify the equality remaining on cdeque sequences. *)
  try first [ apply flattenp_Sn_m | apply flattenp_n_Sm ];
  apply flattenp_eq_rect;
  rewrite (map_subst (fun n => @cdeque_seq _ n _));
  reflexivity ].
...
green_of_red (Big (Triple p1 (Triple p2 child s2 PCYellow) s1 PCRed) cd CCRed)
  ...
  | ? (p1', Any p2'), ? (Any s2', s1') :=
    ...
    let cd' := eq_rect _ (fun n => lvl_cdeque A n _) cd _ (f_equal S (eq_sym (plus_n_Sm _ lvl))) in 
...
green_of_red (Big (Triple p1 Hole s1 PCRed) (Big (Triple p2 child s2 PCGreen) cd CCGreen) CCRed)
  ...
  | ? (p1', Yellowish p2'), ? (Yellowish s2', s1') :=
    (* let C := get_color cd in 
    let len := get_length child in *)
    let cd' := eq_rect _ (fun n => lvl_cdeque A n _) cd _ (f_equal S (plus_n_Sm _ lvl)) in 
