...
Inductive incr_nat_eq : nat -> nat -> nat -> Type := 
| Zcase (nlvl lvl : nat) : nlvl = lvl -> incr_nat_eq nlvl 0 lvl 
| Scase (nlvl len lvl : nat) : S nlvl = S len + lvl -> incr_nat_eq (S nlvl) (S len) lvl.
...
Equations eq_add_succ (n m : nat) : n = S m -> { p : nat | n = S p } := 
eq_add_succ 0 m e := False_rec _ (O_S m e);
eq_add_succ (S n) m e := ? m.

Search JMeq.

Equations flattenp_gen {A} (nlvl len lvl : nat) :
    incr_nat_eq nlvl len lvl -> 
    list (prodN A nlvl) -> 
    list (prodN A lvl) := 
flattenp_gen lvl 0 lvl (Zcase lvl lvl eq_refl) := id;
flattenp_gen (S lvl) 1 lvl (Scase lvl _ lvl eq_refl) := flattenp;
flattenp_gen (S nlvl) (S (S len)) lvl (Scase _ _ lvl e) with eq_add_succ nlvl (len + lvl) (eq_add_S nlvl (S len + lvl) e) => {
    | ? p := (flattenp_gen (S p) (S len) lvl (Scase p len lvl _)) ∘ flattenp }. 
Next Obligation.
  cbn. intros H A * e%(eq_add_S).
  induction len.
  - cbn in e. destruct e. constructor. reflexivity.
  - rewrite e. apply Scase. apply (f_equal S). reflexivity.
Qed.
Arguments flattenp_gen {A nlvl len lvl}.
Search flattenp_gen_obligations_obligation_1.
...
cdeque_seq (Big p cd e _) := 
    packet_front_seq p ++
    flattenp_gen e (cdeque_seq cd) ++
    packet_rear_seq p.
...
buffer_push x (B5 a b c d e) := 
    ? Big (Triple (B3 x a b) Hole (B3 c d e) PCGreen) (Small B0) (Scase lvl 0 lvl eq_refl) _.
Next Obligation.
  cbn. intros. 
...
buffer_inject (B5 a b c d e) x := 
    ? Big (Triple (B3 a b c) Hole (B3 d e x) PCGreen)(Small B0) eq_refl _.
...
make_small prefix1 buf suffix1 with (prefix_decompose prefix1), (suffix_decompose suffix1) => {
  ...
  | (? Underflow opt), (? Overflow s1 ab) with suffix_rot buf ab => {
    | ? (cd, center) with prefix23 opt cd => {
      | ? prefix => ? Big (Triple prefix Hole s1 PCGreen) (Small center) eq_refl _ } };
  | (? Overflow p1 ab), (? Underflow opt) with prefix_rot ab buf => {
    | ? (center, cd) with suffix23 cd opt => {
      | ? suffix => ? Big (Triple p1 Hole suffix PCGreen) (Small center) eq_refl _ } };
  | (? Overflow p1 ab), (? Overflow s1 cd) with buffer_halve buf => {
    | ? (x, Any rest) with suffix12 ab x => {
      | ? prefix => ? Big (Triple p1 (Triple prefix Hole (B1 cd) PCYellow) s1 PCGreen) (Small rest) eq_refl _ } }
}.
Next Obligation. Qed.
Next Obligation. Qed.
Next Obligation. Qed.
...
Equations plus_n_Sm : forall n m, S (n + m) = n + S m := 
plus_n_Sm 0 m := eq_refl;
plus_n_Sm (S n) m := f_equal S (plus_n_Sm n m).
Transparent plus_n_Sm.
...
Lemma test {B C : Type} (n1 n2 : nat) (e : n1 = n2) (h : B) (f : B -> C) :
  (match e with | eq_refl => f end) h = match e with | eq_refl => f h end.
Proof.
...
green_of_red (Big (Triple p1 (Triple p2 child s2 PCYellow) s1 PCRed) cd eq_refl CCRed)
  ...
  | ? (p1', Any p2'), ? (Any s2', s1') :=
    let e2 := plus_n_Sm _ lvl in
    ? Big (Triple p1' Hole s1' PCGreen) (Big (Triple p2' child s2' PCRed) cd e2 CCRed) eq_refl CCGreen };
green_of_red (Big (Triple p1 Hole s1 PCRed) (Big (Triple p2 child s2 PCGreen) cd eq_refl CCGreen) eq_refl CCRed)
  ...
  | ? (p1', Yellowish p2'), ? (Yellowish s2', s1') :=
    let e := eq_sym (plus_n_Sm (S _) lvl) in 
    ? Big (Triple p1' (Triple p2' child s2' PCYellow) s1' PCGreen) cd e CCGreen }.
Next Obligation.
