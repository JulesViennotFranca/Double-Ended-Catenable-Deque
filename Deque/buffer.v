...
Equations push_vector {A n q} (v : vector A n) (b : t A q) :
    { b' : t A (vector_length v + q) | seq b' = vector_seq v ++ seq b } :=
push_vector V0 b := ? b;
push_vector (V1 a1) b with push a1 b => { | ? b' := ? b' };
push_vector (V2 a1 a2) b with push2 a1 a2 b => { | ? b' := ? b' };
push_vector (V3 a1 a2 a3) b with push2 a2 a3 b => {
  | ? b' with push a1 b' => { | ? b'' := ? b'' } };
push_vector (V4 a1 a2 a3 a4) b with push2 a3 a4 b => {
  | ? b' with push2 a1 a2 b' => { | ? b'' := ? b'' } };
push_vector (V5 a1 a2 a3 a4 a5) b with push5 a1 a2 a3 a4 a5 b => { | ? b' := ? b' };
push_vector (V6 a1 a2 a3 a4 a5 a6) b with push6 a1 a2 a3 a4 a5 a6 b => { | ? b' := ? b' }.

Equations inject_vector {A n q} (b : t A q) (v : vector A n) :
    { b' : t A (vector_length v + q) | seq b' = seq b ++ vector_seq v } :=
inject_vector b V0 := ? b;
inject_vector b (V1 a1) with inject b a1 => { | ? b' := ? b' };
inject_vector b (V2 a1 a2) with inject2 b a1 a2 => { | ? b' := ? b' };
inject_vector b (V3 a1 a2 a3) with inject2 b a1 a2 => {
  | ? b' with inject b' a3 => { | ? b'' := ? b'' } };
inject_vector b (V4 a1 a2 a3 a4) with inject2 b a1 a2 => {
  | ? b' with inject2 b' a3 a4 => { | ? b'' := ? b'' } };
inject_vector b (V5 a1 a2 a3 a4 a5) with inject5 b a1 a2 a3 a4 a5 => { | ? b' := ? b' };
inject_vector b (V6 a1 a2 a3 a4 a5 a6) with inject6 b a1 a2 a3 a4 a5 a6 => { | ? b' := ? b' }.
Next Obligation.
  cbn; intros * H2 * H1; rewrite H1; hauto db:rlist.
Qed.
Next Obligation.
  cbn; intros * H21 * H22; rewrite H22; hauto db:rlist.
Qed.

Equations push5_vector {A n q} (a1 a2 a3 a4 a5 : A) (vec : vector A n) (b : t A q) :
    { b' : t A (5 + vector_length vec + q) | 
