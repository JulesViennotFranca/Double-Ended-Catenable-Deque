...
packet_front_seq (Yellow buf1 dq _) :=
  buffer_seq buf1 ++ flattenp (packet_front_seq dq);
packet_front_seq (Green buf1 dq _) :=
  buffer_seq buf1 ++ flattenp (packet_front_seq dq);
packet_front_seq (Red buf1 dq _) :=
  buffer_seq buf1 ++ flattenp (packet_front_seq dq).
...
packet_rear_seq (Yellow _ dq buf2) :=
  flattenp (packet_rear_seq dq) ++ buffer_seq buf2;
packet_rear_seq (Green _ dq buf2) :=
  flattenp (packet_rear_seq dq) ++ buffer_seq buf2;
packet_rear_seq (Red _ dq buf2) :=
  flattenp (packet_rear_seq dq) ++ buffer_seq buf2.
...
packet_hole_flatten (Yellow _ dq _) :=
  flattenp ∘ packet_hole_flatten dq;
packet_hole_flatten (Green _ dq _) :=
  flattenp ∘ packet_hole_flatten dq;
packet_hole_flatten (Red _ dq _) :=
  flattenp ∘ packet_hole_flatten dq.
...
cdeque_seq (G dq kont) :=
  packet_front_seq dq ++
  packet_hole_flatten dq (cdeque_seq kont) ++
  packet_rear_seq dq;
cdeque_seq (Y dq kont) :=
  packet_front_seq dq ++
  packet_hole_flatten dq (cdeque_seq kont) ++
  packet_rear_seq dq;
cdeque_seq (R dq kont) :=
  packet_front_seq dq ++
  packet_hole_flatten dq (cdeque_seq kont) ++
  packet_rear_seq dq.
...
deque_seq (T kont) := cdeque_seq kont.
...
Equations buffer_push {A : Type} {C : phantom} (x : A) (b : buffer A C) :
  { k : cdeque A is_green | cdeque_seq k = x :: buffer_seq b } :=
...
Equations buffer_inject {A : Type} {C : phantom} (b : buffer A C) (x : A) :
  { k : cdeque A is_green | cdeque_seq k = buffer_seq b ++ [x] } :=
...
Equations cdeque_of_opt3 {A} (o1 : option A) (o2 : option (A * A)) (o3 : option A) :
  { k : cdeque A is_green |
    cdeque_seq k = option_seq o1 ++ flattenp (option_seq o2) ++ option_seq o3 } :=
...
Equations make_small {A C1 C2 C3}
  ...
  (b3 : buffer A C3) :
  { k : cdeque A is_green |
    cdeque_seq k = buffer_seq b1 ++ flattenp (buffer_seq b2) ++ buffer_seq b3 } :=
make_small prefix1 buf suffix1 with (prefix_decompose prefix1), (suffix_decompose suffix1) => {
  ...
  | (? Ok p1), (? Underflow opt) with buffer_eject buf => {
    | ? None with opt => {
      | None := ? Small p1;
      | Some x with buffer_inject p1 x => { | ? k := ? k } };
  ...
  | (? Underflow opt), (? Ok s1) with buffer_pop buf => {
    | ? None with opt => {
      | None := ? Small s1;
      | Some x with buffer_push x s1 => { | ? k := ? k } };
  ...
  | (? Underflow p1), (? Underflow s1) with buffer_unsandwich buf => {
    ...
    | ? Alone opt with cdeque_of_opt3 p1 opt s1 => { | ? k := ? k } }
...
Equations green_of_red {A : Type} (k : cdeque A is_red) :
  { k' : cdeque A is_green | cdeque_seq k' = cdeque_seq k } :=
...
Equations ensure_green {A C} (ny: not_yellow C) (k : cdeque A C) :
  { k' : cdeque A is_green | cdeque_seq k' = cdeque_seq k } :=
...
Equations yellow {A B: Type} {G1 Y1 Y2 K2 G3 Y3 G4 R4 : phantom}
  ...
  (dq : packet (A * A) B (PColor PNotgreen Y2 PNotred) K2)
  (buf2 : buffer A (PColor G3 Y3 PNotred))
  (k : cdeque B (PColor G4 PNotyellow R4)) :
  { sq : deque A |
    deque_seq sq = buffer_seq buf1 ++
               flattenp (packet_front_seq dq) ++
               flattenp (packet_hole_flatten dq (cdeque_seq k)) ++
               flattenp (packet_rear_seq dq) ++
               buffer_seq buf2 }
:=
...
push x (T (Small buf)) with buffer_push x buf => {
  | ? buf' => ? T buf' };
push x (T (G (Green p1 child s1) kont)) with green_push x p1 => {
  | ? buf' with yellow buf' child s1 kont => {
    | ? sq' => ? sq' } };
push x (T (Y (Yellow p1 child s1) kont))
...
inject (T (Small buf)) x with buffer_inject buf x => {
  | ? buf' => ? T buf' };
inject (T (G (Green p1 child s1) k)) x with green_inject s1 x => {
  | ? buf' with yellow p1 child buf' k => {
    | ? sq' => ? sq' } };
inject (T (Y (Yellow p1 child s1) k)) x
...
pop (T (G (Green p1 child s1) k)) with green_pop p1 => {
  | ? (x, Yellowish p1') with yellow p1' child s1 k => {
    | ? sq' => ? Some (x, sq') } };
pop (T (Y (Yellow p1 child s1) k)) with yellow_pop (Yellowish p1) => {
...
eject (T (G (Green p1 child s1) k)) with green_eject s1 => {
  | ? (Yellowish s1', x) with yellow p1 child s1' k => {
    | ? sq' => ? Some (sq', x) } };
eject (T (Y (Yellow p1 child s1) k)) with yellow_eject (Yellowish s1) => {
...
(* A type for this new structure that is called ldeque. *)
...
(* Get the list represented by a ldeque. *)

Set Equations Transparent.
Equations ldeque_seq {A} : ldeque A -> list A :=
ldeque_seq {| seq_length := len; seq := sq |} with Z_lt_le_dec len 0 => {
...
Equations empty {A} : { sq : ldeque A | ldeque_seq sq = [] } :=
empty := ? {| seq_length := 0; seq := T (Small B0) |}.

(* Emptyness test for ldeque. *)
...
Equations rev {A} (sq : ldeque A) : { sq' : ldeque A | ldeque_seq sq' = rev (ldeque_seq sq) } :=
