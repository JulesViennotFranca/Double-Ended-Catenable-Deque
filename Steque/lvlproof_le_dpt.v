...
Arguments ZElt {A}.
Arguments SElt {A lvl pdpt csdpt dpt C1 C2}.
Arguments Hole {A lvl}.
Arguments Triple {A lvl len pdpt pktdpt sdpt dpt Y C}.
Arguments Small {A lvl sdpt dpt}.
Arguments Big {A lvl len pdpt ndpt dpt nlvl C1 C2 C3}.
...
Equations make_green {A lvl dpt C}
    ...
    (cs : lvl_csteque A (S lvl) dpt C)
    (s : suffix A lvl dpt) :
    { cs' : lvl_csteque A lvl (S (S dpt)) green | csteque_seq cs' = prefix_seq p ++
                                                                csteque_seq cs ++
                                                                suffix_seq s } :=
make_green p (Small s') s := ? Big (Triple p Hole s) _ (Small s') _ _ _;
make_green p (Big pkt _ cs eq_refl _ CCGreen) s :=
    ? Big (Triple p Hole s) _ (Big pkt _ cs _ _ CCGreen) _ _ CCGreen;
make_green p (Big pkt _ cs eq_refl _ CCYellow) s :=
    ? Big (Triple p pkt s) _ cs _ _ CCGreen;
make_green p (Big pkt cs eq_refl _ CCOrange) s :=
    ? Big (Triple p pkt s) cs _ _ CCGreen;
make_green p (Big pkt cs eq_refl _ CCRed) s :=
    ? Big (Triple p Hole s) (Big pkt cs _ _ CCRed) _ _ CCGreen.
Next Obligation.
  intros.
