...
(* A type for sized leveled options. *)
...
(* In the following, [concat_map_***_seq] functions assume the structure
   ...
   Model functions for our structures are just special cases of
   [concat_map_***_seq] functions, where [T] is instanciated with [fun A _ => A]
...
Equations green_opt_push {A lvl sizeb sizeo}
  ...
  { c : chain A lvl (sizeo + sizeb) green |
    chain_seq c = option_seq o ++ buffer_seq b } :=
...
Equations green_opt_inject {A lvl sizeb sizeo}
  ...
  { c : chain A lvl (sizeo + sizeb) green |
    chain_seq c = buffer_seq b ++ option_seq o } :=
...
Equations chain_of_opt3 {A lvl s1 s2 s3}
  ...
  { c : chain A lvl (s1 + s2 + s2 + s3) green |
    chain_seq c = option_seq o1 ++ option_seq o2 ++ option_seq o3 } :=
